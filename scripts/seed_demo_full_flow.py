...
SECTION_CONTENT = {
    "1": """本项目建设单位为示例环保科技有限公司，项目名称为示例树脂生产及配套环保设施提升项目。项目位于示例省示例市环保产业园示例路 18 号，占地面积 18,600 m²，建筑面积 12,400 m²。项目主要产品为示例产品 A 和示例产品 B，设计年产量为 20,000 t/a。\n\n项目所在园区具备道路、供水、供电和污水集中处理等基础条件。建设内容包括生产车间、原料仓库、成品仓库、废气治理区、污水处理站和危废暂存间。企业已确认本项目按环保设施与主体工程同时设计、同时施工、同时投产的原则组织建设。""",
...
def main() -> None:
    ...
    try:
        ...
        kb = db.scalar(select(KnowledgeBase).where(KnowledgeBase.name == "示例项目演示依据库", KnowledgeBase.created_by == USER_ID))
        if kb is None:
            kb = KnowledgeBase(name="示例项目演示依据库", description="用于演示本地 RAG 检索与报告引用。", scope="private", status="active", created_by=USER_ID, organization_id=project.organization_id)
            db.add(kb)
            db.flush()
        ...
        for section in sections:
            ...
            if draft is None:
                ...
                draft = SectionDraft(project_id=PROJECT_ID, document_instance_id=INSTANCE_ID, template_id=instance.template_id, section_id=section.template_section_id, generation_run_id=run.id, content=SECTION_CONTENT.get(section.section_code, "本章节暂无内容。"), ai_original_content=SECTION_CONTENT.get(section.section_code, "本章节暂无内容。"), status="approved", version=1, citations=citations, missing_information=[], warnings=[], generation_metadata={"provider": "mock", "model": "demo-chat-model", "prompt_version": "section-v1", "demo_seed": True}, created_by=USER_ID)
                db.add(draft)
                db.flush()
